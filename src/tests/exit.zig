...
test "exit" {
    ...
    const path = try allocator.dupeZ(u8, @import("@build_options").prog_exit_path);
    ...
    if (libbpf.bpf_object__next_program(obj, null)) |prog| {
        ...
        try testing.expectStringEndsWith(aw.getWritten(), "trace_printk: error occur at exit.zig:5 return 1");
    }
}
