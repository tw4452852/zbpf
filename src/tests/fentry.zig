...
test "fentry" {
    ...
    const entry_prog = libbpf.bpf_object__find_program_by_name(obj, "fentry").?;
    const exit_prog = libbpf.bpf_object__find_program_by_name(obj, "fexit").?;

    const map = libbpf.bpf_object__find_map_by_name(obj, "ret").?;
    ...
    const expected = buf.len +| n;

    const k: u32 = 0;
    var got: usize = undefined;
